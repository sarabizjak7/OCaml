...
module Peano_axioms : NAT = struct
	type t = Z | S of t
	let rec eq x y =
		match (x, y) with
			| (Z, Z) -> true
	...
	let substraction = failwith "later"
	let multiplication = failwith "later"
	let to_int x = failwith "later"
	let from_int x = failwith "later"
